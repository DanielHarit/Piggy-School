...
export const updateCreditCardByChildrenId = async (childrenId, cardId) => {
	const resultUpdate = await db.collection(collectionName).updateOne({ _id: childrenId }, { $set: { CardId: cardId } });
...
export const updateWishList = async (childrenId, wishes) => {
	const resultUpdate = await db.collection(collectionName).updateOne({ _id: childrenId }, { $set: { WishList: wishes } });
