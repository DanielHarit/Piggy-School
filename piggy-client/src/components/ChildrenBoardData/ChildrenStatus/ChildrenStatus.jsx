...
const ChildrenStatus = ({ child, lastDate }) => {
	...
	const percent = useMemo(
		() => cardData?.amount && Math.min((userExpenses / (cardData?.amount + userExpenses)) * 100, 100),
		[userExpenses, cardData]
	);

	useEffect(async () => {
		const userCard = await axios.get(`${config.PAYMENT_SERVICE_URL}/card/${child._id}`);
		setCardData(userCard.data);
		const userTotalBudget = Object.values(child.Budget).reduce((sum, categoryAmount) => sum + +categoryAmount, 0);
		...
		<div className={classes.root}>
			<Box sx={{ display: 'flex', justifyContent: 'space-between', width: '100%' }}>
				...
				<Typography className={classes.targetText}>
					{/* ₪ {cardData?.amount + userExpenses} /{userExpenses} */} {userExpenses}/{userBudget}₪
				</Typography>
			</Box>
			<Box sx={{ width: '100%' }}>
