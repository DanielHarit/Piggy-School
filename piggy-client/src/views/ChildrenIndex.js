...
const ChildIndex = () => {
  ...
  const { showWatchPrizePopup } = useStories()
  const { toggleSetShowWatchPrizePopup } = useStoriesUpdate()

  useEffect(() => {
    if (showWatchPrizePopup) {
      Swal.fire({
        title: 'הרווחת!',
        ...
        toggleSetShowWatchPrizePopup(false)
        setUser({ ...user, PiggyCoins: user.PiggyCoins + 12 })
      })
    }
  }, [showWatchPrizePopup])
      ...
      <HomePage title="PIGGY">
        ...
        <div className={classes.root}>
          <Routes>
            ...
            <Route
              path={routes.Stories + '/:storyPrefix'}
              element={<Stories />}
            />
